...
# --- 1b. GPU NVIDIA dans Docker (optionnel) ----------------------------------
echo "GPU NVIDIA (Docker, optionnel)"
if ! command -v nvidia-smi >/dev/null 2>&1; then
  warn "pas de GPU NVIDIA détecté (la stack tournera en CPU)"
elif command -v nvidia-ctk >/dev/null 2>&1; then
  ok "nvidia-container-toolkit présent (GPU exposable aux conteneurs)"
else
  warn "GPU NVIDIA présent mais nvidia-container-toolkit absent — GPU non visible dans Docker"
  hint "bootstrap.sh installe le toolkit ; voir docs/DEPENDENCIES.md"
fi
...
if command -v bwrap >/dev/null 2>&1; then
  ok "bubblewrap (sandbox du code Blender généré)"
else
  warn "bubblewrap absent — le code Blender généré tourne SANS confinement (AAC_BLENDER_SANDBOX=auto) ou échoue (=require)"
  hint "installer le paquet 'bubblewrap' (bootstrap.sh le pose)"
fi

# --- 5. SearXNG (web) --------------------------------------------------------
echo "SearXNG (web)"
if [ -f docker/searxng/settings.yml ]; then ok "docker/searxng/settings.yml"
else warn "docker/searxng/settings.yml absent (généré par 'make setup')"; hint "make setup"; fi

...
def _render_preview(exe: str, request: BlenderRequest) -> str | None:
    ...
    render_script = (
        ...
        f'\n'
        f'# -- Moteur de rendu : EEVEE explicite pour preview rapide et reproductible --\n'
        f'# BLENDER_EEVEE_NEXT (Blender 4.x transition) est revenu à BLENDER_EEVEE en 5.x.\n'
        f'# On tente les deux pour être robuste quelle que soit la version installée.\n'
        f'_eevee_engines = ["BLENDER_EEVEE", "BLENDER_EEVEE_NEXT"]\n'
        f'_available_engines = bpy.context.scene.render.bl_rna.properties["engine"].enum_items.keys()\n'
        f'for _eng in _eevee_engines:\n'
        f'    if _eng in _available_engines:\n'
        f'        bpy.context.scene.render.engine = _eng\n'
        f'        break\n'
        f'\n'
        f'# -- Résolution fixe 512x512 pour preview stable --\n'
        ...
        f'\n'
        f'# -- Clipping caméra adapté au radius pour éviter les coupures --\n'
        f'cam_obj.data.clip_start = max(0.01, distance * 0.01)\n'
        f'cam_obj.data.clip_end = max(distance * 10.0, 100.0)\n'
        f'\n'

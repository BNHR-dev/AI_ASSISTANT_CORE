...
container as on Linux (WSL2 backend), so you get the exact same confinement — `cap_drop: ALL`,
`no-new-privileges`, read-only rootfs, no extra privileges. Nothing is reimplemented for
Windows.
...
`scripts\windows\Install-AAC.bat` installs everything **natively, without Docker** (winget:
Git, Python, Ollama + models, Blender, ComfyUI; SearXNG still needs Docker Desktop).
...
```bat
scripts\windows\Install-AAC.bat              REM full install (self-elevates)
